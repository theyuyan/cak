...
if (cmd === 'front') {
  ...
  if (flag('default')) { const v = flag('default')!; const known = ['tui', 'tty', 'web', ...installedFronts()]; if (!known.includes(v)) { console.error(`没有前端 ${v}（可选：${known.join(' / ')}；cak front --list 看清单）——没有改默认`); process.exit(1); } fs.mkdirSync(path.dirname(cfgFile), { recursive: true }); fs.writeFileSync(cfgFile, JSON.stringify({ ...readCfg(), front: v }, null, 1) + '\n'); console.log(`默认前端 → ${v}`); process.exit(0); }
  ...
  else { const mp = path.join(os.homedir(), '.cak', 'plugins', id, 'manifest.json'); if (!fs.existsSync(mp)) { const fromCfg = !(specPath && !specPath.startsWith('--')) && readCfg().front === id; console.error(`未安装前端 ${id}${fromCfg ? `（config 里的默认前端指向它；改回：cak front --default tui）` : `（cak add ${id} 安装，或 cak front --list 看有哪些）`}`); process.exit(1); } const m = JSON.parse(fs.readFileSync(mp, 'utf8')); if (!(m.roles ?? []).includes('frontend')) { console.error(`${id} 不是前端插件`); process.exit(1); } const c = spawn(m.entrypoint.command, [...(m.entrypoint.args ?? []), ...rest], { stdio: 'inherit', ...(m.cwd ? { cwd: m.cwd } : {}) }); c.on('close', code => process.exit(code ?? 0)); }
  await new Promise(() => {});
}
if (cmd === 'doctor') {
