...
/** 相对路径、不含 ..、不是盘符；URL（http://…）也不以 / 开头，desktop.open 这类「路径或网址」入参也能用 */
const PATH_OK = '^(?![/\\\\])(?!.*(^|[/\\\\])\\.\\.([/\\\\]|$))(?![A-Za-z]:).*$';
export type PluginGrant = { contract: string; version?: string; sideEffects: string; /** 需要加路径墙的入参名（true = 'path'；兼容旧调用） */ pathArg?: boolean | string[] };
