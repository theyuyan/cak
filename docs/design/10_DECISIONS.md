...
| N-33 | 第一个真"外部插件"路径走通：`@cak-dev/sdk` 做成可打包的独立 package（`sdk/package.json`，本地 tarball 安装；发 npm 等公开）；create-cak-plugin 生成 → 实现 → 子进程 conformance → 注册表条目 → `cak add` → cak-code 默认装载 `~/.cak/plugins`（插件契约按 sideEffects 定审批：read/none 免审，其余审批，可 s 常设放行）。同时修两处内核 bug（16 §3-2）：内置契约目录按模块位置定位（CLI 曾只能在仓库根跑）；>16KB 结果 view 从 blob 补回（曾让控制器看到 null、模型重试 5 次） | 用户「丰富插件满足不同用户」+ 业界最常见工具（Fetch/Search/Browser/DB/Memory/GitHub）；先做 http.fetch。"只有我写的插件用过"掩盖了这三处 |
...
| N-36 | 插件代码仓库：私有 monorepo `theyuyan/cak-plugins`（`plugins/<id>/`，随仓库 vendored `@cak-dev/sdk` tarball 直到发 npm）；注册表条目加 `install:{type:'git',url,ref,subdir,build?}`，`cak add` = clone --depth 1 → subdir 里跑 build（argv 数组不经 shell，默认 npm install + npm run build）→ 该 cwd 起子进程过 conformance → manifest 记 cwd → 装载同 cwd。三个插件已从 GitHub 重装验证 | 用户问「插件有库了？」：注册表在 GitHub 但代码只在本机、条目是本机绝对路径=换机器装不到。生态成熟前 monorepo 省事，有外部作者再拆 |
...
| N-39 | 第二个物种：`sdk-python/`（零依赖，标准库）与 `@cak-dev/sdk` 逐字对齐 `cak/1` 线协议（hello/health/shutdown/execute/cancel，-32700/-32600/-32601/-32602/-32603）；execute 在线程里跑（Provider 可阻塞）；示例 `text.summarize` 插件过内核 conformance 13/13、经注册表 git 源 `cak add` 装入、DeepSeek 驱动的 cak-code 真调用（账本 providerId=py-summarize）。协议才是接口，SDK 只是方便 | 用户问「是否真贯彻万物皆 agent」：只有 TS SDK 时谈不上"万物"。跨语言证明的是：内核只认契约与 conformance，不认语言 |
...
| N-47 | 形状对齐用户主线「装内核 → 起内核 → 插件管理装零件 → 搭出自己的 agent」：**agent 配置 = AgentSpec YAML**（`~/.cak/agents/<name>.yaml`，内置 bare / coding / review；cak-code 退化为 coding profile）；host 按 profile 组装，控制器/后端从内置表 + 已装 **in-process 插件（T2）** 取（清单 `entrypoint.type: in-process` 早已定义）；`cak up`（默认 bare 空内核）/ `cak agent list|show|init`；动态部分（已装能力契约、记忆源、注册表、审查方）由 `mergeDynamic` 合入不写回文件。内核零改动 | 用户 08-19「装内核→插件管理→搭自己的 agent，与万物 agent 是否违背/能否完成/会否改架构」：不违背（世界 vs 节点）、能（compose(spec, plugins) 本来就是这个）、不改架构（宿主/CLI/安装器层）。真跑：bare 起来自述只有插件管理与历史→装 doc-read→热加载 |
| N-48 | 子进程控制器：传输 METHODS **只增** `controller.decide`（内核→插件 {decideId, view, config}）与反向 `ctx.invoke/compose/preview/attenuate/spawn`（插件→内核，带 decideId）；`SubprocessProvider.onRequest` 处理反向请求；`SubprocessController` 把 ControllerContext 搬到线上；SDK `servePlugin(null, {controller})`；已装子进程插件 hello.roles 含 controller 即成为可选控制器（T1，不进内核进程）。子进程里 preview 只有异步版 `invokePreview` | 补上进程内控制器（T2）的信任缺口；控制器也能用任何语言写。用户「你决定」；transport 属只增，16 §3 允许 |
| N-49 | **内核进程 = 内核服务 + 0..N 个 agent**：daemon 重构为可零 agent 运行（`cak up --no-agent`），控制面提供 `kernel.status / plugins.list|search|install|registry / agents.list|add|remove`（插件与配置管理不依赖任何模型；装完所有 agent 热加载），`session.*` 与 SSE 带 `agent` 维度；`cak` 一个词默认顺手挂一个 bare（引导 agent，可摘可换）；`cak agent add/remove/loaded` 对着运行中的内核。内核零改动 | 用户问「起内核是起小 agent 还是纯内核」→ 我建议「纯内核 + 引导 agent」，用户「根据你的建议来」。真跑：纯内核 → 控制面装 py-summarize（无模型）→ agents.add bare → 立刻能用 text.summarize |
| N-50 | **社区契约随注册表分发**：`FileRegistry.contracts()` 读 `<registry>/contracts/**/*.json`；`installPlugin` 的 conformance 已知集 = 内置 + 注册表 + extra；宿主 compose 把注册表契约作 `plugins.contracts` 传入（origin=plugin）；`cak conformance --contracts DIR|FILE` 与 `~/.cak/registry/contracts` 同理。同 name@version 不同 digest → `CAPABILITY_CONTRACT_CONFLICT`，绝不静默覆盖。内核 `contracts/builtin/` 只留内核自己那 34 个。内核 Stable API 零改动（api-surface 不变） | 08-19 并行开发 5 个新插件、10 个新契约：若仍要求进内核 builtin，每加一个插件都得发一版 CLI——「插件名录（含契约目录）」这个仓库的定位本来就是放契约的 |
| N-51 | **同进程兄弟 agent 互相委派**：daemon 里的 `SiblingRouter` 把每个 agent 的 `agent.invoke` 目标表指向同进程其他 agent（动态 Proxy：agents.add/remove 即时生效、排除自己）；只接受 contract=`agent.task@1`（intent+context）；走 daemon 自己的排队/审批链（被委派方要审批照样弹给前端），**不走 Kernel.serve**（那是跨进程、要名片与句柄令牌的路）；回执 = 被委派方账本 `taskReceipt`；委派深度 ≥3 拒绝；grant 由 `mergeDynamic({siblings})` 加、external 默认审批。控制器 SIBLING_RULE 告诉模型怎么用。内核零改动 | 「万物皆 agent」在同一内核里最该有的一块——之前 agent.invoke 只能打跨进程的 cak serve；用户「继续强大插件生态」时我判断委派比再堆插件更值。真跑：bare 把写文件委派给 coding，两边审批各弹各的，hello.py 落盘、report 回到 bare |
| N-52 | **技能（skill）以插件形态存在**：技能 = SKILL.md（frontmatter name/description/requires）+ 附件，只给模型看；注册表条目 `roles:[skill]` + `entrypoint:none` → `installPlugin` 只 clone、tier **T0**、带可执行入口拒装；`skills` 能力插件（cak-plugins）三处发现（~/.cak/skills · 工作区 .cak/skills · 已装 skill 条目）提供 `skill.list`/`skill.read`；宿主有 `skill.list` 就自动挂成上下文源（清单每轮注入）+ 控制器 SKILL_RULE（对得上先读再做）；读取进账本。内核零改动 | 用户问「agent 有 skills，我们这个系统 skills 是不是也可以用插件的方式存在」——之前没显式设计；技能是知识不是能力，做成纯文本 T0 最省事也最安全，脚本类需求交给能力插件本身 |
