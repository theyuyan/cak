// 真驱动 cak-code 交互版：像人一样坐在终端前——看到提示才输入。用法：node tests/drive-repl.mjs <workspace> [scenario.json] [cli 额外参数…]
...
const extra = process.argv.slice(4);   // 其余参数透传给 cli（如 --plugins-dir DIR --no-mcp）
const child = spawn('node_modules/.bin/tsx', ['apps/cak-code/cli.ts', '--workspace', ws, '--session', 'drive-' + Date.now(), ...extra], { stdio: ['pipe', 'pipe', 'pipe'], env: process.env });
